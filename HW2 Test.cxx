...
void placeLine(char** board, int rows, int cols, const string& player,
    ...
    // Check if it's a dot or already occupied
    if (board[row][col] != ' ') {
        invalidMove = true;
        invalidRow = row;
        invalidCol = col;
    ...
    if (row % 2 == 0 && col % 2 == 1 && row >= 2) {
        char owner = checkBox(board, row - 1, col, rows, cols);
        if (owner != ' ') {
            board[row - 1][col] = (owner >= 'a' && owner <= 'z') ? 
                                   (char)(owner - 32) : owner;
            // Add box to player
    ...
    if (row % 2 == 1 && col % 2 == 0 && col >= 2) {
        char owner = checkBox(board, row, col - 1, rows, cols);
        if (owner != ' ') {
            board[row][col - 1] = (owner >= 'a' && owner <= 'z') ? 
                                   (char)(owner - 32) : owner;
            int idx = findPlayer(playerList, playerCount, player);
            if (idx == -1) {
                Player* newList = new Player[playerCount + 1];
...
char checkBox(char** board, int boxRow, int boxCol, int rows, int cols) {
    ...
    if (top != ' ' && bottom != ' ' && left != ' ' && right != ' ') {
        // Return the last player who completed it (lowercase)
        char lastPlayer = top;
        if (bottom > lastPlayer || (lastPlayer == ' ')) lastPlayer = bottom;
        if (left > lastPlayer || (lastPlayer == ' ')) lastPlayer = left;
        if (right > lastPlayer || (lastPlayer == ' ')) lastPlayer = right;
        
        // Convert to lowercase
        if (lastPlayer >= 'A' && lastPlayer <= 'Z') {
            return (char)(lastPlayer + 32);
        }
        return lastPlayer;
    }
